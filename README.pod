...
C<NQP> can run on four different backends: C<MoarVM>, the C<JVM>, C<JavaScript>,
and C<.NET>.
All backends except for C<MoarVM> are currently experimental.  The C<JVM> backend
requires C<JDK 1.9> (also known as C<JDK 9>) or higher. The C<.NET> backend
requires C<.NET 5>.
